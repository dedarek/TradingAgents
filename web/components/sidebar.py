"""Sidebar: ticker input, LLM config, history list."""
...
def _render_llm_config() -> None:
    ...
    if provider_key in MODEL_OPTIONS:
        ...
        st.text_input("快速思考模型 ID", key="custom_quick_model")
        st.text_input("深度思考模型 ID", key="custom_deep_model")
        st.session_state["quick_think_llm"] = st.session_state.get("custom_quick_model", "")
        st.session_state["deep_think_llm"] = st.session_state.get("custom_deep_model", "")
...
def render_sidebar() -> None:
    st.markdown(
        ...
        <div style="text-align:center; margin-bottom:1.5rem;">
            <span style="font-size:2rem; font-weight:800; color:#ff5a1f;">Trading</span><span style="font-size:2rem; font-weight:800; color:#f5f1eb;">Agents</span>
    ...
    ticker = st.text_input(
        "股票代码",
        placeholder="例: SPY, AAPL, 0700.HK, BTC-USD",
        key="input_ticker",
        help="输入 Yahoo Finance 支持的股票代码，带交易所后缀",
    )
